...
> **범위 안내.** 이 리뷰는 커밋 `ab71988` 시점의 저장소 전체 — 완전한 브리지 `v2`, 축소판 `ExportV2`, 터미널 엔진 `Export` — 를 대상으로 쓴 것이다. 이후 저장소는 5절의 결론대로 **`ExportV2` 만 독립 패키지로 남기도록 축소**됐고, `v2` 와 `Export` 는 git 이력에서만 볼 수 있다. 본문의 `v2/bridge/...` 경로는 그 커밋 기준이다. 4절의 문제점 중 4.2(기본 권한)는 축소 시 예시 설정 기본값을 `default` 로 바꿨고, 4.4(추출 로직 중복)는 `ExportV2` 가 어댑터 한 벌만 갖게 되면서 해소됐다. 나머지는 여전히 유효하다.
