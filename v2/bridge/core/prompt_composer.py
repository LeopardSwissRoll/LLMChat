...
class PromptComposer:
    ...
    def compose(
        ...
        if mode_level == 1:
            ...
            # LV2-4: load @@1 through @@(mode+2-1), cut before @@(mode+2)
            persona_text = self._extract_before_section(full_text, mode_level + 2)
        else:
            # LV5 (masquerade): full PROMPT.md
            persona_text = full_text

        # Resolve the @레퍼런스 marker into the persona folder path.
    ...
    def _extract_before_section(text: str, section_num: int) -> str:
        """Extract everything before the given section number."""
        for m in SECTION_RE.finditer(text):
            if int(m.group(1)) >= section_num:
                return text[:m.start()].strip()
        return text  # section not found → return all
    ...
    def _resolve_reference_marker(text: str, persona_dir: Path) -> str:
        """Replace the @레퍼런스 marker with a single absolute folder path."""
